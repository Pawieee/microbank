...
# conn = create_engine("sqlite:///backend/database.db")
...
def assess_loan_eligibility(employment_status:String, monthly_income:float, income_source:String, is_in_debt:bool, ):
    # ADD THE NECESSARY LOAN INFO HERE
    salary = request.form.get("salary")
    cred_score = request.form.get("credit_score")
    emp_status = request.form.get("employment_status")

    salary_range = [8000, 15000, 25000, 35000, 50000]
    employment_status_table = [{"employed":5}, {"self-employed":3}, {"unemployed":1}]

    salary_range_score = 0
    for i in range(len(salary_range)):
        if salary <= salary_range[i]:
            salary_range_score += 1
        else:
            break

    loan_plans = [{"1":2500}, {"2":5000}]
