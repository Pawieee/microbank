...
import {
  User, CreditCard, Building2, Wallet,
  AlertCircle, Loader2, BadgeCheck, ShieldAlert,
  Maximize2, Banknote, FileText, XCircle, ChevronLeft, ChevronRight,
...
const REJECTION_REASONS = [
  ...
  "Unable to Verify Employment Information",
...
export function Release(props: ReleaseProps) {
  ...
  const { isManager, isTeller, user } = useAuth();
  ...
  // ✅ FIX: Separate states for Selection and Text Input
  ...
  const onReject = async () => {
    // ✅ COMBINE LOGIC: Use selected reason OR custom remarks OR both
    let finalReason = "";

    if (selectedReason) {
        finalReason = selectedReason;
        // If they also typed something, append it for context
        if (customRemarks && customRemarks.trim().length > 0) {
            finalReason += ` - ${customRemarks.trim()}`;
        }
    } else {
        // If no button selected, rely purely on text input
        finalReason = customRemarks.trim();
    }

    // Validation
    if (!finalReason) {
        triggerAlert({ title: "Input Required", description: "Please select a reason or enter specific remarks.", variant: "destructive" });
        return;
    }

    const result = await processRejection(finalReason);
  ...
  const handlePrint = () => {
    processPrintLog();
    setIsPrinted(true);
    const finalTotalRepayment = displayData.amount || amount;
    ...
    printLoanAgreement({
      ...
      total_repayment: finalTotalRepayment,
      payment_schedule: displayData.payment_schedule || "Monthly",
      duration: duration,
      date_applied: date_applied,
      manager_name: user?.fullName || "Authorized Officer"
    });
  };
  ...
  // Helper to check if rejection form is valid
  const isRejectionValid = selectedReason || (customRemarks && customRemarks.trim().length > 0);
              ...
              <div className="flex-1 overflow-y-auto p-8 bg-white">
                <div className="max-w-md mx-auto flex flex-col gap-6">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {/* ✅ UPDATED: Clicking these now only sets 'selectedReason' */}
                    {REJECTION_REASONS.map((reason) => (
                        <Button 
                            key={reason} 
                            variant="outline" 
                            className={cn(
                                "justify-start h-auto py-3 px-4 text-left font-normal border-zinc-200 hover:border-red-300 hover:bg-red-50 hover:text-red-700", 
                                selectedReason === reason && "border-red-500 bg-red-50 text-red-800 ring-1 ring-red-500"
                            )} 
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Other Reason / Specific Remarks</Label>
                    {/* ✅ UPDATED: Binds only to 'customRemarks' */}
                    <Textarea 
          ...
          <div className="p-6 border-t bg-zinc-50 shrink-0">
            {viewMode === 'view-rejected' ? (
              ...
              viewMode === 'review' ? (
                ...
                <div className="flex justify-between items-center w-full">
                    <Button variant="ghost" onClick={() => setViewMode('review')}><ChevronLeft size={16} className="mr-1" /> Back</Button>
                    {/* ✅ VALIDATION: Check both selectedReason AND customRemarks */}
                    <Button 
                        variant="destructive" 
                        onClick={onReject} 
                        disabled={!isRejectionValid}
                    >
                        Confirm Rejection
                    </Button>
                </div>
