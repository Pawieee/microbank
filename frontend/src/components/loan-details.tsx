...
import { IconArrowLeft, IconCalendar, IconCreditCard } from "@tabler/icons-react";
  ...
  return (
    <div className="w-full px-6 py-6 space-y-6 bg-gray-50/50 min-h-screen flex flex-col">
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-end mb-3 gap-2">
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-bold text-gray-900">₱{totalPaid.toLocaleString()}</span>
              <span className="text-gray-400 font-medium">/ ₱{amount.toLocaleString()}</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 w-full">
        <StatCard
          label="Next Due Date"
          value={new Date(next_due).toLocaleDateString("en-PH", { month: "short", day: "numeric", year: "numeric" })}
          icon={<IconCalendar className="w-4 h-4 text-gray-400" />}
        />
        <StatCard label="Due Amount" value={`₱${Number(due_amount).toLocaleString()}`} highlight />
        <StatCard label="Principal" value={`₱${principal.toLocaleString()}`} />
        <StatCard
          label="Duration"
          value={`${duration} Months`}
          subtext={`Ends: ${new Date(new Date(start_date).setMonth(new Date(start_date).getMonth() + duration)).toLocaleDateString("en-PH", { month: "short", year: "numeric" })}`}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
        ...
        {/* LEFT: Loan Particulars */}
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 h-full">
          <h3 className="text-base font-semibold text-gray-900 mb-6 flex items-center gap-2">
            <div className="w-1 h-5 bg-blue-500 rounded-full"></div>
            Loan Particulars
          </h3>
          {/* Reduced to 4 items (2x2 grid) */}
          <div className="grid grid-cols-2 gap-y-8 gap-x-6">
            <DetailItem label="Start Date" value={new Date(start_date).toLocaleDateString("en-PH", { year: "numeric", month: "long", day: "numeric" })} />
            <DetailItem label="Payment Schedule" value={payment_schedule} />
            <DetailItem label="Application Date" value={new Date(date_applied).toLocaleDateString("en-PH", { year: "numeric", month: "long", day: "numeric" })} />
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 h-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-8 gap-x-6">
            ...
            <DetailItem label="Credit Score" value={credit_score ? credit_score.charAt(0).toUpperCase() + credit_score.slice(1) : "N/A"} />
          </div>
        </div>
      </div>
